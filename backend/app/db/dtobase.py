...
from app._types import Order, QueryParams
from app.db.database import Base, sessionmanager
from app.exceptions import ExistError, NotFoundError
from app.utils.context import locale_ctx

if TYPE_CHECKING:
    from sqlalchemy.engine.interfaces import ReflectedForeignKeyConstraint, ReflectedUniqueConstraint

    from app.db.mixins import AuditLog
...
async def inspect_table(table_name: str) -> InspectorTableConstraint:
    ...
    if result := TABLE_PARAMS.get(table_name):  # noqa: PGH003 # type: ignore
        return result
