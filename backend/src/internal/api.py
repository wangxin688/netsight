...
from src.features.admin.models import Permission
from src.features.deps import AuthUser, SqlaSession

router = APIRouter()


@router.get("/permissions", operation_id="8057d614-150f-42ee-984c-d0af35796da3")
async def get_permissions(session: SqlaSession, user: AuthUser) -> list[schemas.Permission]:  # noqa: ARG001
...
async def sync_db_permission(session: SqlaSession, user: AuthUser, request: Request) -> dict:  # noqa: ARG001
