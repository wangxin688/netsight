...
from src.features.consts import DeviceStatus, RackStatus
from src.features.dcim import schemas
from src.features.dcim.models import Device, DeviceType, Rack
from src.features.org.models import Location
from src.features.org.services import LocationDto
...
class RackDto(BaseRepository[Rack, schemas.RackCreate, schemas.RackUpdate, schemas.RackQuery]):
    async def create(
        self,
        session: "AsyncSession",
        obj_in: schemas.RackCreate,
...
class DeviceDto(BaseRepository[Device, schemas.DeviceCreate, schemas.DeviceUpdate, schemas.DeviceQuery]):
    async def create(
        self,
        session: "AsyncSession",
        ...
        if obj_in.location_id and not obj_in.site_id:
            db_loc = await loc_dto.get_one_or_404(session, obj_in.location_id)
            obj_in.site_id = db_loc.site_id
        device_type_dto = BaseRepository(DeviceType)
