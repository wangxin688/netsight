...
from netsight.features.circuit.services import circuit_service, isp_service
...
class IspAPI:
    ...
    async def create_isp(self, isp: schemas.ISPCreate) -> IdResponse:
        new_isp = await self.service.create(self.session, isp)
    ...
    async def update_isp(self, id: int, isp: schemas.ISPUpdate) -> IdResponse:
        db_isp = await self.service.get_one_or_404(self.session, id)
        await self.service.update(self.session, db_isp, isp)
        return IdResponse(id=id)

    @router.get("/isp/{id}", operation_id="9c132285-b785-41b9-b3c9-9d13320dcd1a")
    async def get_isp(self, id: int) -> schemas.ISP:
        db_isp = await self.service.get_one_or_404(self.session, id, selectinload(ISP.asn).load_only(ASN.id, ASN.asn))
    ...
    async def delete_isp(self, id: int) -> IdResponse:
        db_isp = await self.service.get_one_or_404(self.session, id)
        await self.service.delete(self.session, db_isp)
        return IdResponse(id=id)
...
class CircuitAPI:
    ...
    async def create_circuit(self, circuit: schemas.CircuitCreate) -> IdResponse:
        site_a_id, device_a_id = await self.service.get_interface_info(self.session, circuit.interface_a_id)
        new_circuit = await self.service.create(self.session, circuit, commit=False)
        ...
        new_circuit = await self.service.commit(self.session, new_circuit)
    ...
    async def update_circuit(self, id: int, circuit: schemas.CircuitUpdate) -> IdResponse:
        db_circuit = await self.service.get_one_or_404(self.session, id)
        if circuit.interface_z_id:
            site_z_id, device_z_id = await self.service.get_interface_info(self.session, circuit.interface_z_id)
        ...
        await self.service.update(self.session, db_circuit, circuit, excludes={"interface_a_id", "interface_z_id"})
        return IdResponse(id=id)
    ...
    async def get_circuit(self, id: int) -> schemas.Circuit:
        db_circuit = await self.service.get_one_or_404(
            self.session,
            id,
            ...
            selectinload(Circuit.device_a).load_only(Device.id, Device.name, Device.management_ip),
    ...
    async def get_circuits(self, q: schemas.CircuitQuery = Depends()) -> ListT[schemas.Circuit]:
        count, results = await self.service.list_and_count(
            ...
            selectinload(Circuit.site_a).load_only(Site.id, Site.name, Site.site_code),
            selectinload(Circuit.device_a).load_only(Device.id, Device.name, Device.management_ip),
            selectinload(Circuit.interface_a).load_only(Interface.id, Interface.name, Interface.description),
            selectinload(Circuit.site_z).load_only(Site.id, Site.name, Site.site_code),
            selectinload(Circuit.device_z).load_only(Device.id, Device.name, Device.management_ip),
            selectinload(Circuit.interface_z).load_only(Interface.id, Interface.name, Interface.description),
        )
    ...
    async def delete_circuit(self, id: int) -> IdResponse:
        db_circuit = await self.service.get_one_or_404(self.session, id)
        await self.service.delete(self.session, db_circuit)
        return IdResponse(id=id)
